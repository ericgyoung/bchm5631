...
# }
intersect_pk = function(peaks) {
  pl_ov = GenomicRanges::findOverlaps(peaks)
  pl1 <- peaks[unique(pl_ov@from)]
  pl2 <- peaks[unique(pl_ov@to)]
  output <- GenomicRanges::reduce(union(pl1, pl2))
  return(output)
}
overlaps = lapply(peak_list,intersect_pk)
