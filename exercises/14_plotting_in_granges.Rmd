...
library(ggpubr)
library(tidyverse)
# source("/scratch/Shares/rinnclass/CLASS_2022/<your_folder>/CLASS_2022/util/plotting_functions.R")
# source("/scratch/Shares/rinnclass/CLASS_2022/<your_folder>/CLASS_2022/util/_setup.R")
...
8) THEME -- a set version of colors, background. Mostly so you don't have to add many of the layers above every time!
...
num_peaks_df <- read_csv('/Users/eryo3113/CLASS2022/EricY/bchm/exercises/analysis/12_peak_features/num_peaks_df.csv')
...
lm_fit <- lm(total_peak_length ~ num_peaks,data = num_peaks_df)
summary(lm_fit)
# slope = 1,000 
# ORDER MATTERS: if reversed the slope is 8 e -3!
lm_fit <- lm(num_peaks ~ total_peak_lengthdata = num_peaks_df)
summary(lm_fit)
...
backgrounds, grid lines etc. We tend to use paper white as a default theme.

We made a default theme that we can source at the beginning of the
