...
summary(table(peaks_per_dbp$dbp))
...
hist(peaks_per_dbp$num_peaks, breaks = 60) 
dev.off()
...
ppdbp_subset2 <- peaks_per_dbp[peaks_per_dbp$dbp == "POLR2A" | peaks_per_dbp$dbp == "POLR2B" | peaks_per_dbp$dbp == "SUPT5H",]
...
peaks_per_dbp[peaks_per_dbp$dbp %in% genes,]
...
Use what we learned above to plot a histogram of all the proteins that are in the medium sized category for total genome coverage.

Make a table with just the medium coverage DBPs
```{r mutate if else statement}
hist(peaks_per_dbp[peaks_per_dbp$genome_coverage_size == "medium",]$total_peak_length)


summary(table(peaks_per_dbp[peaks_per_dbp$genome_coverage_size == "medium",]$dbp))
